...
import {
  IResourceTypeControllerReadResourceTypeOutputError as IControllerError,
  IResourceTypeControllerReadResourceTypeOutputSuccess as IControllerSuccess,
...
/**
 * @name					resourceTypeRouterReadResourceType
 *
 * @description		The 'readResourceType' router for 'resourceType', whose expected
 *                inputs have been defined within the following schema:
 *                'resourceTypeValidationSchemaReadResourceType'.
 *
...
export const resourceTypeRouterReadResourceType = Router(options).get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'resourceTypeValidationSchemaReadResourceType'.
      ...
      const stat = await resourceTypeController.readResourceType(validatedParams);
       ...
       * 'resourceTypeModel' from 'stat.payload' and return
       * it with an appropriate status code.
       */

      const { resourceTypeModel, responseStatusCode } = (stat as IControllerSuccess)
        .payload;
      res.status(responseStatusCode).send({ resourceTypeModel });
    } catch (error) {
      /*
