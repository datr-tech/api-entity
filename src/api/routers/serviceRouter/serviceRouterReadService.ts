...
import {
  IServiceControllerReadServiceOutputError as IControllerError,
  IServiceControllerReadServiceOutputSuccess as IControllerSuccess,
...
/**
 * @name					serviceRouterReadService
 *
 * @description		The 'readService' router for 'service', whose expected
 *                inputs have been defined within the following schema:
 *                'serviceValidationSchemaReadService'.
 *
...
export const serviceRouterReadService = Router(options).get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
       ...
       * defined within 'serviceValidationSchemaReadService'.
      ...
      const stat = await serviceController.readService(validatedParams);
       ...
       * 'serviceModel' from 'stat.payload' and return
       * it with an appropriate status code.
       */

      const { serviceModel, responseStatusCode } = (stat as IControllerSuccess).payload;
      res.status(responseStatusCode).send({ serviceModel });
    } catch (error) {
      /*
